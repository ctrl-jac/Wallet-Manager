...
def withdrawal_view(request):
    ...
    if not response_data['message']:
        ...
        if wallet.balance - int(transaction.amount) < 0:
            try:
                transaction.type = 'WX'
                transaction.status = 'ABT'
                transaction.save()
                response_data['status'] = 'FAILED'
                response_data['message'].append('Insufficient Funds.')
            except ValueError as e:
                logger.error('Error in Deposit : DB Error <Attempted State = TRANSACTION_ABORT>' + str(e.args[0]))
                response_data['status'] = 'FAILED'
                response_data['message'].append('Database Error')
            ...
            try:
                ...
                try:
                    transaction.type = 'WX'
                    transaction.status = 'CMP'
                    transaction.transaction_time = timezone.now()
                    transaction.save()
                    response_data['status'] = 'SUCCESS'
                    withdraw = {"status": "success",
                                "withdrawn_by": str(wallet.customer.cxid),
                                "withdrawn_at": str(transaction.transaction_time),
                                "amount": str(amount),
                                "reference_id": str(reference_id)
                                }
                    ...
                    logger.error(
                        'Error in Deposit : DB Error <Attempted State = TRANSACTION_COMPLETE>' + str(e.args[0]))
                    response_data['status'] = 'FAILED'
                    response_data['message'].append('Database Error')
            except ValueError as e:
                logger.error('Error in Deposit : DB Error <Attempted State = WALLET_UPDATE>' + str(e.args[0]))
